...
export function CanonWorkspace() {
  ...
  return <section className="page-shell canon-page"><header className="page-title"><span className="scribble">THE OFFICIAL-ISH RECORD</span><h1>Confirm the canon.</h1><p>AI did the boring part. You just nod, rename, or slap a “disputed” sticker on it.</p></header>
    <div className="canon-board">
      <div className="event-stack">{events.map((event, index) => <article className={`event-polaroid tilt-${index % 3} ${event.disputed ? "disputed" : ""}`} key={event.id}><div className="event-number">CASE #{String(events.length - index).padStart(3,"0")}</div><h2>{event.title}</h2><div className="event-meta"><span>📍 {event.place}</span><span>🗓️ {event.date}</span><span>🌡️ {event.mood}</span></div><blockquote>“{event.quote}”</blockquote><div className="face-pile">{event.participantIds.map((id) => <span key={id} style={{background:person(id)?.color}} title={person(id)?.name}>{person(id)?.emoji}</span>)}</div><button className="dispute-button" onClick={() => canonStore.toggleDisputed(event.id)} type="button">{event.disputed ? "⚠ DISPUTED (perfect)" : "✓ Looks canon"}</button>{event.disputed && <div className="disputed-stamp">DISPUTED!</div>}</article>)}</div>
